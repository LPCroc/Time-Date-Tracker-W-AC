...
// NOTE: Time is shown ONLY on INPUT (no Context header, no Output footer/narration).

(function () {
  const VERSION = "DTT 1.4.3 (remove prose toggles)";
  ...
  // (No more Lock/Hide toggles — card only shows Display Time + Minutes per action)
...
Minutes per action: ${rate}`;
  ...
  // ---------- Read settings from the single Time card ----------
  ...
  function applyManualFromCard(state,storyCards){
    try{
      ...
      const beforeMin=state.clockMinutes, beforeDay=state.dayNumber; 
  ...
  function handleViewTurn(state,info){
    init(state); 
    const tid=getTurnId(info,state), last=state._dtt.lastSeen;
  ...
  // No header in Context (still syncs card & rewind guard)
  ...
  // No footer/narration in Output (no prose-time locking/hiding anymore)
